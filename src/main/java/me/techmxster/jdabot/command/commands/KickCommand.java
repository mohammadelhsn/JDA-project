...
public class KickCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        if(!member.canInteract(target) || !member.hasPermission(Permission.KICK_MEMBERS)) {
            channel.sendMessage("You are missing the permissions to kick this member!").queue();
            return;
        }
        ...
        if(!selfMember.canInteract(target) || !selfMember.hasPermission(Permission.KICK_MEMBERS)) {
            channel.sendMessage("I cannot kick this member!").queue();
            return;
        }

        final String reason = String.join(" ", args.subList(1, args.size()));

        ctx.getGuild().kick(target, reason).reason(reason).queue((__) -> channel.sendMessage("Kick was successful!").queue(),
